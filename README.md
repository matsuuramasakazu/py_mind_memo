...
*   **アイコン表示**: トピックの横に視覚的なアイコンを設定し、情報をより直感的に強調・整理できます。
...
*   **Undo / Redo**: 誤った操作（トピックの追加・編集・削除、配置変更、参照関係の変更など）を `Ctrl+Z` で元に戻し、`Ctrl+Y` でやり直すことができます（最大10回分）。
*   **自動保存**: 一度ファイル名を入力して保存した後は、編集内容が定期的に自動で保存されます。不意のトラブルによるデータ損失を防ぎます。
*   **テンプレート機能**: 用意されたテンプレートから素早く新しいマインドマップの作業を開始できます。
*   **操作マニュアル**: メニューバーの「File」→「操作マニュアル (Ctrl+M)」から、アプリの操作方法を記したマインドマップをいつでも参照できます。マニュアルは読み取り専用のモーダルウィンドウで表示されます。
*   **フォントサイズ変更**: `Ctrl+ホイール` でマップ全体のフォントサイズを直感的に拡大・縮小（`Ctrl+0` で標準サイズにリセット）可能。
...
| **Ctrl + I (選択時)** | トピックに **アイコン (PNG)** を追加（assets/iconsから選択） |
| **Ctrl + Z** | 直前の操作を **元に戻す (Undo)**（最大10回まで） |
| **Ctrl + Y** | 取り消した操作を **やり直す (Redo)** |
| **Ctrl + N** | 用意されたテンプレートから新しくマインドマップを作成 |
| **Ctrl + M** | **操作マニュアル** をマインドマップ形式で表示（読み取り専用） |
...
| **Ctrl + 0** | マップのフォントサイズを **標準サイズ (10pt) にリセット** |
...
| **Ctrl + ホイール** | マップのテキストフォントサイズを **拡大 / 縮小**（6pt〜32pt）。 |
